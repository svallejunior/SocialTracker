...
export async function GET(req: NextRequest) {
  try {
    ...
    profiles.forEach(p => {
      ...
      const isMorreu = statusUpper.includes('MORREU') || statusUpper.includes('MORTO') || statusUpper.includes('FALECID');

      // Se a conta está marcada como 'MORREU', a última data de resultado consultado é a que vale (não repete até hoje)
