...
import GraficoSemanasRegistradas from "../components/GraficoSemanasRegistradas";
...
export default function Dashboard() {
  ...
  const [unselectedProfiles, setUnselectedProfiles] = useState<Set<string>>(new Set());
  ...
  const filteredPosts = useMemo(() => posts.filter(post => {
    ...
    // Se houver perfil específico selecionado no dropdown select:
    if (selectedProfileFilter !== 'Todos') {
      if ((post.username || '').toLowerCase() !== selectedProfileFilter.toLowerCase()) {
        return false;
      }
    }

    // Filtro por botões: por default todos marcados; ao desmarcar, aquele resultado some da lista
    const postUser = (post.username || '').toLowerCase();
    const isMinha = minhasModelosSet.has(postUser);

    if (isMinha) {
      if (unselectedProfiles.has(postUser)) {
        return false; // desmarcada -> some da lista
      }
    } else {
      if (unselectedProfiles.has('__DEMAIS__')) {
        return false; // demais modelos desmarcadas -> somem da lista
      }
    }
  ...
  }), [posts, searchQuery, selectedFormats, unselectedProfiles, selectedProfileFilter, minhasModelosSet, startDate, endDate]);
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ====================================================
          ABA: ANÁLISE (QUADRO ANÁLISE DE PERFIL + GRÁFICOS: PROJEÇÃO / SEMANAS)
          ==================================================== */}
      {activeTab === 'graficos' && (
        <>
          ...
          <div className="followers-history-box">
            <div style={{ display: 'flex', gap: 6, background: '#161B22', padding: 4, borderRadius: 8, border: '1px solid #30363D', width: 'fit-content', marginBottom: 20 }}>
              {([
                ['projecao', '🎯 Projeção'],
                ['semanas', '📅 Semanas Registradas']
              ] as const).map(([aba, rotulo]) => (
                <button
                  key={aba}
                  onClick={() => setAbaGrafico(aba)}
                  style={{
                    padding: '6px 14px',
                    borderRadius: 6,
                    border: 'none',
                    ...
                    fontWeight: 700,
                    cursor: 'pointer',
                    background: abaGrafico === aba ? '#7100E2' : 'transparent',
                    color: abaGrafico === aba ? 'white' : '#8B949E',
                    transition: 'all 0.2s'
                  }}
                >
                  {rotulo}
                  ...
                  .map(p => {
                    ...
                    return (
                      <button
                        ...
                        title={isChecked ? `Clique para desmarcar e ocultar @${p.username}` : `Clique para marcar e exibir @${p.username}`}
                        style={{
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: '6px',
      ...
      {activeTab === 'crm' && (
        <CentralCRM
          profiles={profiles.filter(p => Number(p.meu_perfil) === 1 || p.meu_perfil === true)}
          controleData={controleData}
          onAbrirLancamento={(l, u) => {
            setLancamentoSelecionado(l);
            setModalLancamento({ username: u, tipo: l.tipo });
          }}
          onNovoLancamento={(u) => {
            setLancamentoSelecionado(null);
            setModalLancamento({ username: u, tipo: 'recebido' });
          }}
          onFinanceiroAlterado={fetchControle}
        />
      )}
